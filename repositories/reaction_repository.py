...
class ReactionRepository(BaseRepository):
    """Лайки постов (таблица post_reactions)."""

    def count_likes(self, post_id: int) -> int:
        """Текущее число лайков у поста."""
        return (
            self.conn.execute(
                text(
                    """
    ...
    def user_has_like(self, user_id: int, post_id: int) -> bool:
        """True, если пользователь уже поставил лайк этому посту."""
        return (
            self.conn.execute(
                text(
                    """
    ...
    def toggle_like(self, user_id: int, post_id: int) -> bool:
        """Переключает лайк; возвращает True, если после вызова лайк стоит."""
        if self.user_has_like(user_id, post_id):
            self.conn.execute(
                text("DELETE FROM post_reactions WHERE user_id = :uid AND post_id = :pid"),
                {"uid": user_id, "pid": post_id},
            )
            return False
        self.conn.execute(
